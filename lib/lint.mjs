...
export const RULES = ['em-dash', 'emoji', 'inline-header-bullet', 'bold-density',
  'header-in-short-reply', 'table', 'horizontal-rule', 'negative-parallelism',
  'trailing-offer', 'hedge', 'ai-vocab', 'length'];
...
function mask(src) {
  ...
  // CommonMark fences close with the same character and at least the opening
  // length. A shorter nested sample must not expose the rest of a code block.
  let fence = null;
  s = s.split(/(?<=\n)/).map((line) => {
  ...
  s = s.replace(/(?<!`)(`+)(?!`)[\s\S]*?(?<!`)\1(?!`)/g, blank);
  s = s.replace(/^(?: {4}|\t).*$/gm, blank);     // indented code
  s = s.replace(/"(?:\\.|[^"\\\r\n])*"|“[^”\r\n]*”|‘[^’\r\n]*’/g, blank);
  // Destinations can contain nested parentheses or spaces in angle brackets.
  // Preserve labels; mask only a balanced destination and its optional title.
  for (let i = 0; i < s.length - 1; i++) if (s[i] === ']' && s[i + 1] === '(') {
    let depth = 1, angle = false, end = i + 2;
    for (; end < s.length && depth; end++) {
      if (s[end] === '\\') { end++; continue; }
      if (s[end] === '<') angle = true;
      else if (s[end] === '>') angle = false;
      else if (!angle && s[end] === '(') depth++;
      else if (!angle && s[end] === ')') depth--;
    }
    if (!depth) { s = s.slice(0, i) + blank(s.slice(i, end)) + s.slice(end); i = end - 1; }
  }
...
export function lint(text, opts = {}) {
  ...
  // Machine output is a contract, not prose. Never ask the agent to rewrite
  // JSON string values or punctuation to satisfy a chat preference.
  let structured = cfg.format === 'structured';
  if (/^\s*[\[{]/.test(text)) {
    try { const value = JSON.parse(text); structured ||= value !== null && typeof value === 'object'; } catch {}
  }
  const masked = structured ? '' : mask(text);
  const wc = words(masked);
  const v = [];

  // --- mechanical preferences; context can still justify an exception ---
  ...
  v.push(...collect(masked, INLINE_HEADER_BULLET, 'inline-header-bullet', 'hard',
    'Bullet opens with bold text. Consider ordinary prose unless emphasis is useful.'));
  ...
  if (cfg.format === 'chat' && wc < cfg.headerFreeUnder) {
  ...
  for (let i = v.length - 1; i >= 0; i--) if (disabled.has(v[i].rule)) v.splice(i, 1);
  ...
  return {
    ...
    hard,
    soft,
    score: score(v, wc, budget),
    scoreVersion: SCORE_VERSION,
    format: structured ? 'structured' : cfg.format,
    budget: cfg.format === 'chat' ? budget : null,
...
function score(v, wc, budget) {
  ...
  s += Math.min(10, n('hedge') * 2);
  s += n('table') ? 3 : 0;
  s += n('horizontal-rule') ? 2 : 0;
  s += n('length') ? Math.min(15, Math.max(0, wc - budget) / 40) : 0;
