/**
 * Measure your own agent's writing.
 *
 * Reads the transcripts already on this machine, extracts the messages that
 * ended a turn (the ones a human actually read), and scores them. Nothing is
 * uploaded, nothing is sent anywhere, no model is called. It is a local file
 * read and some regexes.
 *
 * This exists because "your agent writes too much" is an opinion until you see
 * your own median.
 */

import { readFileSync, readdirSync, statSync, existsSync } from 'node:fs';
import { join } from 'node:path';
import { lint } from './lint.mjs';
import { CLAUDE_PROJECTS_DIR, CODEX_SESSIONS_DIR } from './paths.mjs';
import { requestContext } from './transcripts.mjs';

const DEFAULT_BUDGET_MB = 400;

function walk(dir, out = [], depth = 0) {
  if (depth > 8) return out;
  let entries;
  try {
    entries = readdirSync(dir, { withFileTypes: true });
  } catch {
    return out;
  }
  for (const e of entries) {
    const p = join(dir, e.name);
    if (e.isDirectory()) walk(p, out, depth + 1);
    else if (e.name.endsWith('.jsonl')) out.push(p);
  }
  return out;
}

function pickRecent(files, budgetBytes) {
  const stated = [];
  for (const p of files) {
    try {
      const s = statSync(p);
      stated.push({ p, size: s.size, mtime: s.mtimeMs });
    } catch { /* vanished mid-scan */ }
  }
  stated.sort((a, b) => b.mtime - a.mtime);
  const picked = [];
  let used = 0;
  for (const f of stated) {
    if (used + f.size > budgetBytes) continue;
    used += f.size;
    picked.push(f.p);
  }
  return { picked, used, total: stated.length };
}

/** Extract final text and its actual request. Never count tool narration as a final. */
export function parseTranscript(raw, tool) {
  const out = [];
  let prompt = '', pending = null, codexFinal = false;
  const textOf = (content) => typeof content === 'string' ? content : Array.isArray(content)
    ? content.filter((b) => ['text', 'input_text', 'output_text'].includes(b?.type)).map((b) => b.text || '').join('\n') : '';
  const save = () => { if (pending) out.push({ text: pending, prompt, ...requestContext(prompt) }); pending = null; };
  for (const line of raw.split('\n')) {
    let r; try { r = JSON.parse(line); } catch { continue; }
    if (r.isSidechain || r.isMeta) continue;
    if (tool === 'claude') {
      const m = r.message;
      if (r.type === 'user' && m && !m.content?.some?.((b) => b?.type === 'tool_result')) {
        save(); prompt = textOf(m.content);
      } else if (r.type === 'assistant' && m) {
        if (m.content?.some?.((b) => b?.type === 'tool_use')) pending = null;
        else { const t = textOf(m.content); if (t.trim()) pending = t; }
        if (m.stop_reason === 'end_turn') save();
      }
    } else {
      const p = r.payload;
      if (r.type === 'event_msg' && p?.type === 'user_message') { prompt = p.message || ''; codexFinal = false; }
      if (r.type === 'response_item' && p?.role === 'user') {
        const t = textOf(p.content);
        if (t.trim() && !t.includes('<hook_prompt') && !t.startsWith('# AGENTS.md instructions') && !t.startsWith('<environment_context>')) { prompt = t; codexFinal = false; }
      }
      if (r.type === 'response_item' && p?.role === 'assistant' && (p.phase === 'final_answer' || p.channel === 'final')) {
        pending = textOf(p.content); save(); codexFinal = true;
      }
      if (r.type === 'event_msg' && p?.type === 'task_complete' && !codexFinal && p.last_agent_message) {
        pending = p.last_agent_message; save(); codexFinal = true;
      }
    }
  }
  if (tool === 'claude') save();
  return out;
}

function readFinals(path, out, tool) {
  try { out.push(...parseTranscript(readFileSync(path, 'utf8'), tool)); } catch {}
}

export function collect({ budgetMb = DEFAULT_BUDGET_MB, tools = ['claude', 'codex'] } = {}) {
  const budget = budgetMb * 1024 * 1024;
  const sources = [];

  if (tools.includes('claude') && existsSync(CLAUDE_PROJECTS_DIR)) {
    sources.push({ tool: 'claude', dir: CLAUDE_PROJECTS_DIR, parse: (p, out) => readFinals(p, out, 'claude') });
  }
  if (tools.includes('codex') && existsSync(CODEX_SESSIONS_DIR)) {
    sources.push({ tool: 'codex', dir: CODEX_SESSIONS_DIR, parse: (p, out) => readFinals(p, out, 'codex') });
  }

  const per = {};
  for (const s of sources) {
    // Budget is per tool, not split between them. Splitting starved whichever
    // corpus was larger and produced a sample too small to mean anything.
    const files = walk(s.dir);
    const { picked, used, total } = pickRecent(files, budget);
    const finals = [];
    for (const f of picked) s.parse(f, finals);
    per[s.tool] = { records: finals, finals: finals.map((r) => r.text), files: picked.length, filesTotal: total, bytes: used, capped: picked.length < total };
  }
  return per;
}

export function score(finals) {
  const results = finals.map((t) => typeof t === 'string' ? lint(t) : lint(t.text, requestContext(t.prompt)));
  const n = results.length || 1;
  const words = results.map((r) => r.words).sort((a, b) => a - b);
  const scores = results.map((r) => r.score);
  const q = (arr, p) => (arr.length ? arr[Math.round(p * (arr.length - 1))] : 0);
  const hitPct = (rule) => (100 * results.filter((r) => (r.counts[rule] || 0) > 0).length) / n;

  return {
    n: results.length,
    medianWords: q(words, 0.5),
    p90Words: q(words, 0.9),
    maxWords: words[words.length - 1] || 0,
    meanScore: scores.reduce((a, b) => a + b, 0) / n,
    cleanPct: (100 * results.filter((r) => r.clean).length) / n,
    emDashPct: hitPct('em-dash'),
    emDashPerMsg: results.reduce((a, r) => a + (r.counts['em-dash'] || 0), 0) / n,
    boldPct: hitPct('bold-density'),
    bulletPct: hitPct('inline-header-bullet'),
    headerPct: hitPct('header-in-short-reply'),
    tablePct: hitPct('table'),
    offerPct: hitPct('trailing-offer'),
    negParPct: hitPct('negative-parallelism'),
    vocabPct: hitPct('ai-vocab'),
    emojiPct: hitPct('emoji'),
    over250: (100 * results.filter((r) => r.words > 250) .length) / n,
    worst: results
      .map((r, i) => ({ r, text: finals[i] }))
      .sort((a, b) => b.r.score - a.r.score)
      .slice(0, 3),
  };
}
