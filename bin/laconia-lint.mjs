#!/usr/bin/env node
/**
 * laconia-lint — deterministic check for AI-writing tells.
 *
 * No dependencies, no model call. Give it text, get back violations with
 * positions and a score. Used by the Stop hook, and runnable by hand on any
 * file you are about to send someone.
 *
 *   node laconia-lint.mjs --file ponuda.md
 *   node laconia-lint.mjs --text "some prose"
 *   echo "..." | node laconia-lint.mjs --stdin --json
 *
 * Exit 0 = clean or soft-only. Exit 1 = at least one hard violation.
 */
...
function isMain() {
  ...
  return a.includes('laconia-lint');
}
...
if (isMain()) {
  ...
  const run = async () => {
    let text = val('--text');
    const file = val('--file') || argv.find((a) => !a.startsWith('--') && argv[argv.indexOf(a) - 1] !== '--text');
    if (text === undefined && (has('--stdin') || !file)) text = await readStdin();
    else if (text === undefined && file) text = readFileSync(file, 'utf8');

    const r = lint(text || '', { depthRequested: has('--depth') });

    if (has('--json')) {
      process.stdout.write(JSON.stringify(r, null, 2) + '\n');
    } else if (r.clean) {
      process.stdout.write(`clean  ${r.words} words  score ${r.score}\n`);
    } else {
      process.stdout.write(`${r.words} words  score ${r.score}  ` +
        `${r.hard.length} hard, ${r.soft.length} soft\n\n`);
      for (const x of r.violations) {
        const tag = x.severity === 'hard' ? 'HARD' : 'soft';
        process.stdout.write(`  ${String(x.line).padStart(4)}  ${tag}  ${x.rule}\n` +
          `        ${x.message}\n`);
      }
